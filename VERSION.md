...
- **Código muerto eliminado**: `reportStatusLabel()` era una función identidad; eliminada y reemplazada por `{report.status}` directamente.
 ...
 - **Fix: Fondo negro en vista de reportes**: corregido layout del `<body>` y estilos CSS para evitar que el fondo oscuro se muestre alrededor del contenido en vistas cortas.
